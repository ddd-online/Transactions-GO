<template>
  <div class="chart-view">
    ...
    <div class="chart-view-header">
      ...
      <a-button type="text" size="small" @click="showConfig = !showConfig">
        <template #icon>
          <SettingOutlined />
        </template>
        配置
    ...
    <!-- 配置信息 -->
    <div v-if="showConfig" class="chart-view-config">
      <a-collapse :bordered="false" ghost>
        <a-collapse-panel key="1" header="图表配置详情">
            ...
            <a-table-column title="筛选条件">
              <template #default="{ record }">
                <template v-if="record.conditions && record.conditions.length > 0">
                  <a-tag v-for="cond in record.conditions" :key="cond.description" color="purple" style="margin-bottom: 4px;">
                    {{ cond.category }}
